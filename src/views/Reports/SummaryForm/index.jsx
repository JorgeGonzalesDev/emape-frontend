...
import { GetPlanillaResumenConcepto } from "../../../service/summaryForm";
import {
...
const Ticket = () => {
  ...
  const filterSpreadsheet = async (event, row) => {
    const response2 = await GetPlanillaResumenConcepto(row.coD_PERPLAN);
  ...
  const downloadExcel = (dataExport) => {
    var Headers = [["REPORTE PLANILLA RESUMEN SEGUN CONCEPTO"]];
    let nData = [];
    dataExport.forEach((item) => {
      nData.push({
        Código: item?.codigo,
        Concepto: item?.concepto,
        "N° Trab.": item?.no_Trabajadores,
    ...
    XLSX.utils.book_append_sheet(workBook, workSheet, "Planilla Resumen Concepto"); //TITULO dE hoja
    XLSX.write(workBook, { bookType: "xlsx", type: "buffer" });
    XLSX.write(workBook, { bookType: "xlsx", type: "binary" });
    XLSX.writeFile(workBook, "ReportePlanillaResumenConcepto.xlsx"); //NOmbre de archivo
  };
  ...
  return (
    <>
      ...
      <Grid container spacing={1}>
        ...
        <Grid item md={12} sm={12} xs={12}>
          <h1 style={{ color: "red" }}>Planilla Resumen segun Concepto</h1>
         </Grid>
        <Grid item md={12} sm={12} xs={12}>
          <DataGridDemo
            height={"50vh"}
