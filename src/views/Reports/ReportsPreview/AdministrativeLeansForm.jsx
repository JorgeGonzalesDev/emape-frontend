...
import { GetReportePrestamosAdministrativos } from "../../../service/common";

const AdministrativeLeansForm = () => {
  const generatePDF = () => {
    const doc = new jsPDF('p', 'pt', 'a4');
    doc.html(document.querySelector(HojaA4), {
      callback: function (doc) {
        doc.save("Reporte-Prestamos-Administrativos.pdf");
      }
    })
  };
  ...
  const loadData = async () => {
    const response1 = await GetReportePrestamosAdministrativos(fields.mes, fields.annio)
      ...
      <Grid item md={3} xs={12} sm={12}>
        <div style={{ flexGrow: 1 }}>
          <Stack direction="row" spacing={1} xs={{ mb: 1, display: "flex" }}>
            <div>
              <h1>Reporte Prestamos Administrativos</h1>
            </div>
          </Stack>
        </div>
      </Grid>
        ...
        <div className="page" id="boleta-pago">
          <div className="border-header d-flex">
            <div className="container">
              <div className="title">
                <h4>PRESTAMOS ADMINISTRATIVOS - {months[fields.mes]} {fields.annio}</h4>
              </div>
            </div>
          </div>
            ...
            <table className="table-center border-table">
              <thead>
                <tr>
                  ...
                  <th className="th-table" colSpan="1">N° CUOTAS</th>
                  ...
                  dataH.map((data, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td className="td-table" colSpan="1">{data['n_CUOTAS']}</td>
                        <td className="td-table-n" colSpan="1">{data['rentA_MENSUAL'].toFixed(2)}</td>
                      </tr>
                    )
                  })
                }
...
export default AdministrativeLeansForm;